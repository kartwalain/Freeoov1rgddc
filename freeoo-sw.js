...
//   - SCROLL BLANK FIX: contain:paint replaces will-change:transform on .card
//     → GPU VRAM pressure eliminated — no more blank sections on mobile scroll
//   - TAB SWITCH FIX: switchOC, ocs10Switch, ocExpiryChange all deferred to rAF
//     → OC/OCS10 tab switches are visually instant on mobile and tablet
//   - DUPLICATE BUG FIX: Removed second switchOC function overwriting first
//     → WS re-subscription now fires correctly on Nifty↔Sensex tab switch
//   - OCS10 ZERO-HANG FIX: buildOCS10 uses yield+rAF pattern (same as buildOC)
//     → Main thread never blocked on OCS10 rebuild — no hang during WS tick bursts
//   - MODAL SPEED FIX: openFooModal defers overflow:hidden to rAF
//     → Footer legal popup opens instantly — no synchronous layout reflow on tap
//   - LINK SPEED FIX: touch-action:manipulation + -webkit-tap-highlight-color removed
//     → India/Global Analysis links open immediately on mobile tap
//   - BUTTON HOVER FIX: transform:translateY wrapped in @media(hover:hover)
//     → No layout recalc on mobile tap for Analysis section buttons
...
//   - OCS REBUILD FIX: idle timeout 800ms→400ms — faster real-time OCS display
    ...
    event.respondWith(new Response(
      JSON.stringify({
        ...
        features: [
          ...
          'scroll-blank-fix-contain-paint','tab-switch-rAF-instant',
          'ocs10-zero-hang-yield-rAF','modal-gpu-opacity-anim',
          'link-speed-tap-highlight-fix','duplicate-switchOC-removed',
          'ocs-rebuild-idle-400ms','mid-scroll-light-repaint-250ms'
        ]
      }),
